...
- **Layout (PC 1024px+)**:
  ...
  - **Left Sidebar (Sticky)**: `position: sticky; top: 120px;`（ヘッダーの高さを考慮）。
    ...
    - `FEATURE` ラベル、縦書きリードコピー、番号付きタイムラインを細い縦ラインでつなぎ、章扉のような一体ユニットとして見せる。
...
- **Card Design**:
  - **Shape**: 白カード、控えめな角丸、薄い境界線、淡いドット背景、内枠、写真背面の円形アクセント、控えめな区切り線で重なりを表現する。カードシャドウは使わず、特徴カード内では写真とテキストを別カード化せず、斜めカットや強いドット装飾も使わない。
...
- **Hero Section**:
  ...
  - Photo: 
    - 垂直方向の位置を引き上げ、ヘッダー部分まで広げて全面に表示する。
    - 左右の幅も全幅（viewport 100%）とし、画面一杯に広がるダイナミックな表示にする。
    - 角丸を解除（直角）し、画面の端まで写真が届くようにする。
  - Header Elements (Mobile):
    - 左上に白背景の独立したロゴカードを配置する。
    - 右上にダーク背景の正方形/角丸正方形のメニューボタンを配置する。
  - Action Bar (Mobile Sticky):
    - 画面最下部に「電話」「お問い合わせ」「LINE」の3つのボタンを並べた固定バーを配置する。
- **Navigation**:
