...
- **Layout (PC 1024px+)**:
  ...
  - **Left Sidebar (Sticky)**: `position: sticky; top: 120px;`（ヘッダーの高さを考慮）。
    - 構成: 英語タイトル "Feature"、日本語タイトル "当クリニックの特徴"、導入テキスト、番号付きタイムライン。
    - タイムラインは右側カードのIntersectionObserver判定と連動し、現在のカード番号・線・ラベルをアクティブ表示に切り替える。
    - 背景に現行のウォーターマーク（"KUROYANAGI CLINIC"）を控えめに配置。
  - **Right Content (Scrollable)**: 特徴を詳しく紹介する4枚のカードを垂直に配置し、PC幅では各カードを `position: sticky` で段階的に固定する。後続カードほど少し下の `top` 位置で止め、1枚ずつ重なっていくスクロール演出にする。
- **Card Design**:
  - **Shape**: 白の半透明カード、控えめな角丸、薄い境界線、影で重なりを表現する。
  - **Visual**: 高品質な写真、大きな薄青の番号、タイトル、本文、詳細リンクで構成する。
